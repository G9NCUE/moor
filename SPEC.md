...
2. **The address-type enum is closed with no extension point.** ✅ `ADDRESS_TYPE_SET` (index.js:41) is frozen over 7 payment types and enforced at index.js:569. A P2P wallet therefore *cannot* store a peer identity key as an address — the one thing a P2P address book might be expected to hold. Moor works around it via `Contact.username`; the real fix is upstream (§8). Filed as [address-book#6](https://github.com/tetherto/wdk-p2p-address-book/issues/6); **PR [address-book#10](https://github.com/tetherto/wdk-p2p-address-book/pull/10)** adds `hyperdht` to the set, four lines plus a test. Finding 20 is why closing the set buys less than it looks like it does.
...
13. **A worklet's `console.log` goes nowhere — on iOS.** ✅ Host-side `console.log` reaches Metro; the same call inside the Bare worklet does not appear in the Metro log, the Xcode log, or `simctl log stream`. This is worth knowing before you debug anything in there, because absence of output looks exactly like code that never ran — it cost us an hour of reasoning from evidence that did not exist. The way to see inside a module is to keep counters on the instance and expose a method that returns them, then call it over `callModule`. **Scoped 2026-08-23:** on Android, the worklet's unhandled rejections and `logger.error` output reach logcat under the app's process tag (`E/<process>: [ERROR] Unhandled promise rejection in worklet: …`), which is how finding 22 was diagnosed in one read. The blindness is an iOS property.
...
16. **The gasless wallet cannot perform an account's first transaction.** ✅ In token-paymaster mode every method that prices an operation does so *before* the EIP-7702 authorization is signed, and the estimation call has no way to carry one. The bundler simulates an EOA with no code and returns `AA20 account not deployed`. So `transfer()` and `sendTransaction()` cannot make a new account's first spend, and `quoteTransfer()`/`quoteSendTransaction()` fail for any account that has not already transacted — which breaks quote-then-confirm, the obvious shape for a send screen. **The cause is structural, not a missing argument.** `_buildSponsoredUserOperation` already accepts a pre-signed authorization and applies it (`wallet-account-read-only-evm-7702-gasless.js:450`, `:462`), but `_getUserOperationGasCost` (`:626`) is defined on `WalletAccountReadOnlyEvm7702Gasless`, which has no `_ownerAccount` and no `_getAuthorization`: a read-only account holds no key and *cannot* sign an authorization. The signing class inherits its estimator from a class incapable of producing the input the estimator needs, and its three call sites pass `{ nonce }` or nothing (`wallet-account-evm-7702-gasless.js:250`, `:301`, `:339`). `quoteTransfer` shows it most clearly — defined *only* on the read-only class (`:258`) and never overridden, so calling it on a fully-keyed account still runs the keyless version. The way through is `signTransaction(tx)` then `sendTransaction(signedOp)`, which builds the authorization and broadcasts without re-estimating; that is what `lab/t10` does and what moved Moor's first money. **Re-confirmed 2026-08-21 on a freshly generated key holding nothing, which needs no funding at all:** `quoteTransfer()` and `transfer()` both return `AA20`, while `signTransaction()` on the same account in the same run reaches `ERC20: transfer amount exceeds balance` — simulation gets as far as the token only when the authorization is present, so the authorization is the only variable. Filed as [gasless#32](https://github.com/tetherto/wdk-wallet-evm-7702-gasless/issues/32). **Fixed upstream in `1.0.0-beta.4`** (PRs #30 and #31, merged the day before we filed; released 2026-08-21, issue closed 2026-08-30). Re-ran the repro on 2026-08-31 against beta.4: all four paths reach the ERC-20 balance check, no `AA20`. Read-only accounts still get no explicit "delegate first" error.
...
20. **The address book's type validation is asymmetric, so the closed enum cannot enforce the invariant it appears to.** ✅ `_validateAddressRecord` runs only on the local write path (`index.js:492`, `:513`). The autobase apply router (`index.js:90`) does no type validation at all — it enforces address+network uniqueness and the UMA duplicate rule, then inserts — and `schema.js:61` stores `type` as a plain required `string`, not an enum. So an address carrying an unrecognised type, written by one client, replicates into another, is stored, and is served back without complaint. **This is not a defect and not a security issue:** validating inside `apply` would risk diverging or breaking replication, so keeping it out is correct, and no per-type *format* checking hangs off the type either — validation is membership only, so a replicated unknown type bypasses nothing, because there is nothing to bypass. What it does mean is that `ADDRESS_TYPES` is advisory rather than an enforced invariant; a multiwriter book cannot guarantee it holds only the seven types. Recorded because it is the argument for finding 2: adding a type, or an extension point, is backward-compatible by construction and adds no exposure that replication does not already carry. Deployed as the technical core of the [address-book#6 escalation](https://github.com/tetherto/wdk-p2p-address-book/issues/6#issuecomment-5364998758) rather than filed separately, since on its own it has no action attached.
21. **JSON-RPC module parity is three-sided, and the third side has no event path.** ✅ `wdk-core-kotlin`'s read loop has one branch, keyed on `id`; a frame without one is parsed and discarded. pear-wrk-wdk#83 defines `moduleEvent` as a notification that deliberately carries no `id`. So the day #83 and bundler#54 merge, every native host core can call a module and none can hear one. Not a defect yet, since nothing emits on that transport today; a scope gap in the plan on bundler#46, which names only the worklet and the generator. Demonstrated rather than inferred by `flutter/phase0/p0-wire.mjs`, where the event arrives as exactly that frame, and fixed in `flutter/wdk_core_flutter/android/…/WdkCore.kt` with one extra branch. Same shape as finding 15: the host side of worklet → host events is the weak point in both stacks.
22. **The bundler links a hardcoded addon list, and `udx-native` is not on it.** ✅ `linkAddons` (on by default for JSON-RPC) iterates `BARE_LINK_MODULES` in `src/constants.ts`: thirty-odd `bare-*` packages plus `sodium-native`. `udx-native`, the UDP transport under `hyperdht` and so under every Holepunch module including `@tetherto/wdk-p2p-address-book`, is absent. The bundle is packed expecting `linked:libudx-native.1.21.1.so`, the APK never contains it, and the module's first network call dies with `ADDON_NOT_FOUND` (`flutter/wdk_core_flutter/README.md`). React Native never sees this because `react-native-bare-kit` ships `libudx-native.so` itself; the Kotlin core never saw it because finding 14 kept modules out of JSON-RPC bundles entirely. The fix is one entry in the list, or better, deriving the list from the bundle's own `linked:` references rather than maintaining one by hand. Worked around in `flutter/phase0/link-udx.mjs`.
23. **A void module method fails over JSON-RPC.** ✅ The shared module runtime returns `{ result: undefined }` for a method that returns nothing; rn-core's host guards for it (`moduleService.ts:86`), pear-wrk-wdk#83's handler calls `JSON.parse(undefined)` and answers with an error. The address book's `create()`, `editContact()` and `deleteContact()` all return nothing, so the book cannot be enrolled over JSON-RPC. One-line fix, committed on our checkout of #83 (`flutter/pear@cd166b1`) and verified by `flutter/phase0/a-wire.mjs`. `@moor/pay-requests` never hit it because all three of its methods return objects.
24. **The worklet drops `error.cause`, on both transports.** ✅ `createStructuredError` serialises `error.message` and, in development, `error.stack`; never `cause`. WDK's wallets wrap the real reason as `cause`: a refused quote reaches the app as `pm_getPaymasterData failed` while the wallet knew `validator: token balance lower than the required 0x7a07 allowance`. `lab/t10` carries its own unwrapper for this; every integrator will write one, or show the wrong message. Found from the Flutter send screen on 2026-08-24; the same frame reaches rn-core.
...
WDK's bundler has two transports. `hrpc` targets React Native; `jsonrpc` targets Swift and Kotlin, which is the only path a Flutter plugin could take. **`modules:` works on `hrpc` and is silently dropped on `jsonrpc`** (finding 14) — and `modules:` is precisely how the P2P address book gets into the worklet. No modules, no contacts, no M1, no M2.

**Revised 2026-08-23.** The paragraph above was true on beta.9 and is still true on the published beta.10. It is no longer true of the code in flight: [pear-wrk-wdk#83](https://github.com/tetherto/pear-wrk-wdk/pull/83) and [bundler#54](https://github.com/tetherto/wdk-worklet-bundler/pull/54) wire `modules:` into JSON-RPC, and [`docs/flutter-poc.md`](docs/flutter-poc.md) records a Flutter app on those branches loading `@moor/pay-requests`, calling it, and receiving a payment request as a `moduleEvent` on an Android emulator. Three things had to be built or worked around to get there, and each is a finding: the host cores have no path for worklet → host events (21), the bundler does not link `udx-native` (22), and the emulator cannot be hole-punched so a local DHT stood in for the public one (the run on a physical Android device is still owed). What the POC did **not** need was the list in the previous version of this paragraph: no hand-written entry point, no Dart state layer, no reimplementation of `wdk-react-native-core`. A 150-line Kotlin port of `wdk-core-kotlin` with one extra branch, a 90-line Dart wrapper, and the two upstream PRs were enough.

Moor itself stays React Native. The requirement — one app on both platforms — is met, the address book has only ever been proven on `hrpc`, and `flutter/` is a proof for upstream rather than a second wallet. If Tether merges the two PRs and fixes 21 and 22, Flutter becomes a choice rather than an impossibility, which is what the POC was for.
...
|---|---|
| M0 — Money | Phase 2 — **done**; balance, receive and send all work from the app, an account's first ever send (finding 16) fixed upstream in beta.4, not yet re-tested from the app |
...
**Done from the app's own screen too**, with one hole. Tapping a contact and sending 0.1 USD₮ on the phone delivered exactly 0.1 and cost 0.0088, from an account holding zero wei. An account's *first* send failed until `beta.4` (finding 16: WDK priced a transfer before signing the EIP-7702 authorization). The app is on beta.4 now; a first send from the phone is still to be re-tested.
...
| The gasless wallet cannot make an account's first send | **low** | Finding 16, fixed upstream in beta.4 and verified in Node. Stays here until a first send is re-tested from the phone |
...
- ~~Does the app work around finding 16 or wait for it upstream?~~ Resolved: upstream fixed it in beta.4. Nothing to work around.
